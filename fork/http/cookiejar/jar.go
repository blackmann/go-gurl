...
import (
	...
	"errors"
	"fmt"
	"github.com/blackmann/go-gurl/fork/http/internal/ascii"
	"io"
	"net"
	"net/http"
	"net/url"
...
func (j Jar) AllCookies(writer io.Writer) {
	if serialized, err := json.Marshal(j.entries); err == nil {
		writer.Write(serialized)
	}
}

func (j *Jar) LoadCookies(reader io.Reader) {
	deserialized := make(map[string]map[string]entry)
	if data, err := io.ReadAll(reader); err == nil {
		_ = json.Unmarshal(data, &deserialized)
	}

	j.entries = deserialized
}
